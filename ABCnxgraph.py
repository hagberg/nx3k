...
from ABCgraph import Graph, degree

class nxGraph(Graph):
    def __init__(self, data=None, **attr):
        super(nxGraph, self).__init__(data=data, **attr)
        self.graph = self.data    # dictionary for graph attributes
        self.node = self.n   # empty node attribute dict
        self.adj = self.a   # empty adjacency dict
        # DEPRECATE
        self.edge = self.adj
    ...
    # deprecate? - replace with n in Graph.nodes?
    ...
    def nodes(self, data=False, default=None):
        if data is True:
            ...
            return ((n,val if val is not None else default) for n,val in self.n.data(data))
        else:
            return iter(self.n)
    ...
    def edges(self, nbunch=None, data=False, default=None):
        if nbunch is None:
            if data is True:
                ...
                return ((u,v,wt if wt is not None else default) for (u,v),wt in self.e.data(data))
            else:  # data is False
                return self.e
        else:
            ...
            if data is True:
                ...
                return ((u,v,wt if wt is not None else default) for (u,v),wt in self.e.data(data) if u in bunch or v in bunch)
            else:  # data is False
    ...
    def degree(self, nbunch=None, weight=None):
        if nbunch in self:
            return degree(self, weight)[nbunch]
        if nbunch is None:
            return degree(self, weight)
        else:
            return ((n,d) for (n,d) in degree(self, weight) if n in nbunch)
    ...
    def subgraph(self, nbunch, copy=True):
        ...
        if copy:
            ...
            for n,snbrs in s.a:
                for nbr, d in snbrs.items():
                    if nbr in H.n:
                        H.e._add_edge(n, nbr, d)
            H.graph = self.data
        else:
            H = s
        return H
            ...
            def bunch_iter(nlist, nodes):
                try:
                    ...
                    # capture error for unhashable node.
                        ...
                        raise
            bunch = bunch_iter(nbunch, self.n)
        return bunch
