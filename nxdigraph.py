...
class nxDiGraph(DiGraph):
    ...
    def edges(self, nbunch=None, data=False, default=None):
        if nbunch is None:
            ...
            if data is True:
                return ((u,v,d) for (u,v),d in self.e.items() if u in bunch)
            elif data is not False:
                return ((u,v,d.get(data,default)) for (u,v),d in self.e.items() if u in bunch)
            else:  # data is False
                return ((u,v) for u,v in self.e if u in bunch)
    ...
    def in_edges(self, nbunch=None, data=False, default=None):
        if nbunch is None:
            ...
            bunch = set(self.nbunch_iter(nbunch))
            nodes_nbrs = ((n,nbrs) for n,nbrs in self.pr if n in bunch)
        if data is True:
            return ((n,nbr,d) for n,nbrs in nodes_nbrs for nbr,d in nbrs.items())
        elif data is not False:
            return ((n,nbr,d.get(data, default))
                    for n,nbrs in nodes_nbrs
                    for nbr,d in nbrs.items())
        else: # data is False
            return ((nbr,n) for n,nbrs in nodes_nbrs for nbr in nbrs)
    ...
    def degree(self, nbunch=None, weight=None):
        ...
        if nbunch is None:
            ...
            bunch = set(self.nbunch_iter(nbunch))
            return ((n,d) for (n,d) in self.n.degree(weight=weight) if n in bunch)

    def out_degree(self, nbunch=None, weight=None):
        if nbunch in self:
            (n,d) = next(self.n.out_degree(weight=weight))
            return d
        if nbunch is None:
            return self.n.out_degree(weight=weight)
        else:
            bunch = set(self.nbunch_iter(nbunch))
            return ((n,d) for (n,d) in self.n.out_degree(weight=weight) if n in bunch)

    def in_degree(self, nbunch=None, weight=None):
        if nbunch in self:
            for (n,d) in self.n.in_degree(weight=weight):
                if n == nbunch:
                    return d
        if nbunch is None:
            return self.n.in_degree(weight=weight)
        else:
            bunch = set(self.nbunch_iter(nbunch))
            return ((n,d) for (n,d) in self.n.in_degree(weight=weight) if n in bunch)
    ...
    def to_undirected(self, reciprocal=False):
        ...
        H = nxGraph()
        H.data = deepcopy(self.data)
        H.add_nodes_from(self.n.items())
        H.add_edges_from( (v,u,deepcopy(d))
                for ((u,v),d) in self.e.items()
                if v in self.pr[u])
        return H
    ...
    def subgraph(self, nbunch, copy=True):
        ...
        if copy:
            ...
            # add nodes
            for n in H.n:
                H_succ[n] = {} # fixme factory
                H_pred[n] = {} # fixme factory
            # add edges
            for n in H_succ:
                Hnbrs = H_succ[n]
                for nbr, d in self_succ[n].items():
                    if nbr in H_succ:
    ...
    def has_predecessor(self, u, v):
        return (u,v) in self.in_e
    def has_successor(self, u, v):
        return (u,v) in self.e
    def predecessors(self, n):
        try:
            return iter(self.pr[n])
        except KeyError:
            raise NetworkXError("The node %s is not in the graph." % (n,))
    def successors(self, n):
        try:
            return iter(self.su[n])
        except KeyError:
            raise NetworkXError("The node %s is not in the graph." % (n,))
