...
from nose.tools import (assert_equal, assert_raises, assert_true, raises,
                        assert_not_equal)
import networkx
from ABCnxgraph import nxGraph
from ABCgraph import Nodes
from ABCgraph import Edges
...
class BaseAttrGraphTester(BaseGraphTester):
    ...
    def test_copy_without_data(self):
        ...
        assert_equal(G._succ, self.k3succ)
        assert_equal(G._pred, self.k3pred)
    ...
    def same_attrdict(self, H, G):
        ...
        assert_equal(G.edge[1][2],H.edge[1][2])
        H.add_edge(1,2,foo=old_foo)
        assert_equal(G.edge[1][2],H.edge[1][2])
        old_foo=H.node[0]['foo']
        H.node[0]['foo']='baz'
        assert_equal(G.node,dict(H.node.items()))
    ...
    def different_attrdict(self, H, G):
        old_foo=H[1][2]['foo']
        H.add_edge(1,2,foo='baz')
        assert_not_equal(G.edge[1][2],H.edge[1][2])
        H.add_edge(1,2,foo=old_foo)
        assert_equal(G.edge[1][2],H.edge[1][2])
        old_foo=H.node[0]['foo']
        H.node[0]['foo']='baz'
...
#        assert_equal(G.edge,H.edge)
        ...
        for (u,v),d in G.e.items():
            assert_true(H.e[(u,v)] == d)
        for (u,v),d in H.e.items():
            assert_true(G.e[(u,v)] == d)
#        if not G.is_directed() and not H.is_directed():
#            assert_true(H.adj[1][2] is G.adj[2][1])
#            assert_true(G.adj[1][2] is H.adj[2][1])
...
#        assert_equal(H.name, 'Subgraph of ('+G.name+')')
        ...
        H=G.subgraph([0])
        assert_equal(H._succ,{0:{}})
        H=G.subgraph([])
        assert_equal(H._succ,{})
        assert_not_equal(G._succ,{})
...
class TestGraph(BaseAttrGraphTester):
    ...
    def setUp(self):
        ...
        self.k3nodes=[0, 1, 2]
        self.K3=self.Graph()
        self.K3.succ = self.K3._succ = self.k3succ
        self.K3.pred = self.K3._pred = self.k3pred
        self.K3.adj=self.K3.edge=Adjacency(AtlasUnion(self.K3.succ, self.K3.pred))
        self.K3.node=self.K3._nodes=self.K3._mapping = {}
        self.K3.node[0]={}
        self.K3.node[1]={}
        self.K3.node[2]={}
        self.K3._directed = False
        self.K3.n = Nodes(self.K3)
        self.K3.e = Edges(self.K3)
        self.K3.a = Adjacency(AtlasUnion(self.K3._succ, self.K3._pred))


    def test_data_input(self):
        G=self.Graph(data={1:[2],2:[1]}, name="test")
        assert_equal(G.name,"test")
        res = sorted((n,dict(nbrs.items())) for n,nbrs in G._succ.items())
        assert_equal(res,[(1, {2: {}}), (2, {})])
    ...
    def test_remove_nodes_from(self):
        G=self.K3
        G.remove_nodes_from([0,1])
        assert_equal(G._succ,{2:{}})
        G.remove_nodes_from([-1]) # silent fail

    def test_add_edge(self):
        G=self.Graph()
        G.add_edge(0,1)
        assert_equal(G._succ,{0: {1: {}}, 1: {}})
        assert_equal(G._pred,{0: {}, 1: {0: {}}})
        G=self.Graph()
        G.add_edge(*(0,1))
        assert_equal(G._succ,{0: {1: {}}, 1: {}})
        assert_equal(G._pred,{0: {}, 1: {0: {}}})
    ...
    def test_add_edges_from(self):
        ...
        assert_equal(G._succ,{0: {1:{}, 2:{'weight':3}}, 1: {}, \
                2:{}})
        G=self.Graph()
        G.add_edges_from([(0,1),(0,2,{'weight':3}),(1,2,{'data':4})],data=2)
        assert_equal(G._succ,{\
    ...
    def test_remove_edge(self):
        G=self.K3
        G.remove_edge(0,1)
        assert_equal(G._succ,{0:{2:{}},1:{2:{}},2:{}})
